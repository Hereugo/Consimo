    ...
    function exportt() {
        function save_scripts(wid) {
            ...
            if (wind.type == "input_wind") {
                result['next_step_handlers'] = ((wid in graph[wid])?next_step_handlers(graph[wid][wid]):"");
            }
        ...
        function generate_code_script(wid, vars) {
            ...
            for (let i=0; i<vars_.length; i++) {
                vars__string += vars_[i].substring(1, vars_[i].length - 1) + ((i != vars_.length-1)?", ":"");
            }
            ...
            if (buttons.length == 0)
                if (wind.type == "input_wind")
                    if (wind.message.photo == "")
                        send_message = sMessage(wid, vars['next_step_handlers'], "msg = ", vars__string);
                    else
                        send_message = sPhotoMessage(wid, vars['next_step_handlers'], "msg = ", vars__string);
                else 
                    if (wind.message.photo == "")
            ...
            let func = message(wid, 
                               send_message, 
                               (command_n != ""?command(command_n):""), 
                               ((wind.type == "input_wind")?var_text(wind.variable_name):""));
            console.log(func);
            console.groupEnd();
            return func;
        }
        ...
        function generate_code_config(wid) {
            ...
            var sub_tree = textformat(wid, wind.message.text.replace(/\{\w+\}/, "{}"), buttonformats, photo);
        ...
        var graph = {}, was = {};
        for (let i=0; i<winds.length; i++) graph[winds[i].id] = {};
